...
//            og.setColor(Color.RED);
            ...
            boolean[] action;
            if(agent instanceof NeuroAgent)
                action = ((NeuroAgent)agent).getAction(this, og);
            else if(agent instanceof HumanKeyboardAgent && GlobalOptions.VisualizationOn)
                action = ((HumanKeyboardAgent)agent).getAction(this, og);
            else
                action = agent.getAction(this/*DummyEnvironment*/);
...
//        evaluationInfo.totalNumberOfCoins   = -1 ; // TODO: total Number of coins.
        ...
        evaluationInfo.mushroomsTotal = Mario.gainedMushrooms;
        evaluationInfo.flowersTotal = Mario.gainedFlowers;
        evaluationInfo.blocksHit = Mario.blocksHit;
        evaluationInfo.totalJumpsPerformed = Mario.totalJumps;
...
//        scene.init();
    ...
    public float[] getEnemiesFloatPos(int ZLevel)
    {
        if (scene instanceof LevelScene)
            return ((LevelScene) scene).enemiesFloatPos(ZLevel);
        return null;
    }
    ...
    public Point getMarioPixePos()
    {
        int xPixel = (int)mario.x-mario.xPicO;
        int yPixel = (int)mario.y-mario.yPicO;
    ...
    {
        int xCam = (int) (mario.xOld) - 160;
        int yCam = (int) (mario.yOld) - 120;
        ...
        else
        {
            if (xCam < 0) xCam = 0;
            if (yCam < 0) yCam = 0;
            if (xCam > levelScene.level.width * 16 - 320) xCam = levelScene.level.width * 16 - 320;
            if (yCam > levelScene.level.height * 16 - 240) yCam = levelScene.level.height * 16 - 240;
        }

        return new Point(xCam, yCam);
    }

    public Point getWorldSize()
    {
        return new Point(levelScene.level.width, levelScene.level.height);        
    }
}