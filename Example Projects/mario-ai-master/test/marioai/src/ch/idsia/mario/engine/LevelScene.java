...
//        ret[Environment.HalfObsWidth][Environment.HalfObsHeight] = mario.kind;
    ...
    {
        return enemiesFloatPos(3);
    }

    public float[] enemiesFloatPos(int ZLevel)
    {
        ...
        for (Sprite sprite : sprites)
        {
            ...
            if (sprite.kind >= Sprite.KIND_GOOMBA && sprite.kind <= Sprite.KIND_FIRE_FLOWER) // WBP modified to include fire flowers
            {
                poses.add((float)ZLevelEnemyGeneralization(sprite.kind,ZLevel));
                poses.add(sprite.x);
                poses.add(sprite.y);
            }
        }
...
//        drawStringDropShadow(g, "#########", 0, 1, 7);
        ...
        drawStringDropShadow(g, "FLOWERS  : " + df.format(Mario.gainedFlowers), 0, 6, 4);          drawStringDropShadow(g, "Blocks Hit : " + Mario.blocksHit, 17, 5, 4);
                                                                                                   drawStringDropShadow(g, "Total Jumps: " + Mario.totalJumps, 17, 6, 4);
...
//            System.err.println("warning: progress line inaccuracy");
    ...
    private static void drawLine(Graphics g, int x1, int y1, int x2, int y2, Color c)
    {
    ...
    public static void drawRect(Graphics g, int x, int y, int width, int height, Color c)
    {
        Color temp = g.getColor();
        g.setColor(c);
    ...
    public static void drawLine(Graphics g, Point startPoint, Point end, boolean endPointIsDeltaValues, Color c)
    {
        if(endPointIsDeltaValues)
            drawLine(g, startPoint.x, startPoint.y, startPoint.x + end.x, startPoint.y + end.y, c);
        else
            drawLine(g, startPoint.x, startPoint.y, end.x, end.y, c);
    }
    ...
    public void bump(int x, int y, boolean canBreakBricks)
    {
        ...
        if ((Level.TILE_BEHAVIORS[block & 0xff] & Level.BIT_BREAKABLE) > 0)
        {
            ...
            {
                level.setBlockData(x, y, (byte) 4);
                level.setBlock(x, y, (byte) 4); // added to prevent brick from being hit more than once
            }
            Mario.hitBlock(x, y);
        }
    }
