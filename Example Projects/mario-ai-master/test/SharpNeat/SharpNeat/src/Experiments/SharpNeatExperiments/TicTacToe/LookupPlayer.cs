...
namespace SharpNeatLib.Experiments.TicTacToe
{
	...
	public class LookupPlayer : IPlayer
	{
		...
		public LookupPlayer(IPlayer player)
		{
			this.playerType = player.PlayerType;
			BuildLookupArray(player);
		}
...
//		}
		...
		private void BuildLookupArray(IPlayer player)
		{
			// By convention we require the player to play as 'cross'. Ensure this is so.
			Debug.Assert(player.PlayerType==BoardUnitState.Cross, "player.PlayerType != BoardUnitState.Cross");
				
			// Iterate over every possible board state.
			Board board = new Board();
			board.ResetToNaught();

			// 3^9 = 19,863 possible board states.
			for(int i=0; i<19683; i++)
			{
				// Obtain and store the player's response to the current board state.
				lookupArray[i] = player.GetNextMove(board);

				// Next board state.
				board.IncrementBoardCode();
			}
		}
...
//		}
		...
		public ByteCoord GetNextMove(Board board)
		{
			...
			{	// For each dimension we obtain an index by converting the opponents
				...
				return	lookupArray[	((int)board.BoardState[0,0])
									+	((int)board.BoardState[1,0]*-3)
									+	((int)board.BoardState[2,0]*-9)
									+	((int)board.BoardState[0,1]*-27)
									+	((int)board.BoardState[1,1]*-81)
									+	((int)board.BoardState[2,1]*-243)
									+	((int)board.BoardState[0,2]*-729)
									+	((int)board.BoardState[1,2]*-2187)
									+	  (int)board.BoardState[2,2]*-6561
									+   9840
									];
...
//									];
			...
			{	// Add 1 to the BoardUnitState
				return	lookupArray[  ((int)board.BoardState[0,0]+1)
									+ ((int)board.BoardState[1,0]+1)*3
									+ ((int)board.BoardState[2,0]+1)*9
									+ ((int)board.BoardState[0,1]+1)*27
									+ ((int)board.BoardState[1,1]+1)*81
									+ ((int)board.BoardState[2,1]+1)*243
									+ ((int)board.BoardState[0,2]+1)*729
									+ ((int)board.BoardState[1,2]+1)*2187
									+ ((int)board.BoardState[2,2]+1)*6561					
									];
			}	
		}
