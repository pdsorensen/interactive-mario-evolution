...
namespace SharpNeatLib.Experiments
{
	public class BinaryElevenMultiplexerExperiment : IExperiment
	{
		...
		{
			populationEvaluator = new SingleFilePopulationEvaluator(new BinaryElevenMultiplexerNetworkEvaluator(), activationFn);
		}
		...
		public string ExplanatoryText
		{
			get
			{
				return @"11-Multiplexer experiment with binary value inputs/outputs.
Evaluation consists of testing a network against each of the 2048 (=2^11) possible input combinations.
...
is 2048 if exact answers are given, and the minimum possible fitness while still being correct overall is 
approx. 1024.

To distinguish between correct low scorers and incorrect low scorers an additional value of 10000 is added 
to the score of networks that give a correct response to each of the 2048 test cases.

Max fitness is therefore 12048.
